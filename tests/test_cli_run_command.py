...
def test_cli_run_dry_run_default_writes_plan_but_does_not_move(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cli_run_yes_applies_moves(tmp_path: Path) -> None:
    series_dir = tmp_path / "InputSeries"
    out_root = tmp_path / "Out"
    plan_file = tmp_path / "rename_plan.json"
